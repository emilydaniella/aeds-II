...
public class exercicio3 {
  ...
  public static void ordenaSelecao(Show[] array, int tamanho) {
    File registros = new File("./859238_selecao.txt");
    try {
      FileWriter registro = new FileWriter(registros);
      ...
      registro.write("859238\t" + comparacoes + "\t" + movimentos + "\t" + (tempo / 1_000_000.0) + "\n");
      registro.close();
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public static Show[] readFile() {
    String path = "disneyplus.csv"; // <-- altere o caminho aqui se necessário
    String line = "";
    Show[] temp = new Show[10000];
    int count = 0;

    try (BufferedReader br = new BufferedReader(new FileReader(path))) {
      line = br.readLine(); // ignora cabeçalho
  ...
  public static void main(String args[]) {
    ...
    ordenaSelecao(shows, shows.length);
    sc.close();
  }
}
