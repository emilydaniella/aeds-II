...
int comparaShows(Show a, Show b, int *comparacoes) {
    (*comparacoes)++;
    if (a.date_added.year != b.date_added.year)
        return a.date_added.year - b.date_added.year;

    if (a.date_added.month != b.date_added.month)
        return a.date_added.month - b.date_added.month;

    if (a.date_added.day != b.date_added.day)
...
int dataTitulo(Show *array, int low, int high, int *comparacoes, int *movimentos);


void quicksort(Show *array, int low, int high, int *comparacoes, int *movimentos) {
    if (low < high) {
        int pivot_index = dataTitulo(array, low, high, comparacoes, movimentos);
        quicksort(array, low, pivot_index - 1, comparacoes, movimentos);
        quicksort(array, pivot_index + 1, high, comparacoes, movimentos);
    }
}
...
int dataTitulo(Show *array, int low, int high, int *comparacoes, int *movimentos) {
    ...
    for (int j = low; j < high; j++) {
        if (comparaShows(array[j], pivot, comparacoes) <= 0) {
            i++;
...
{
  FILE *log_file = fopen("859238_quicksort.txt", "a");
  if (log_file == NULL)
  {
...
FILE *log = fopen("859238_quicksort.txt", "w");
if (log) {
