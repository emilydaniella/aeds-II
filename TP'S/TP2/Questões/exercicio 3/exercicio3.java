...
public class exercicio3 {
  ...
  public static void main(String args[]) {
    Show[] shows = readFile();
    Show[] array = new Show[shows.length];
    int tam_array = 0;

    // Parte 1 - Inserção dos registros no vetor
    while (true) {
        ...
        if (currShow != null) {
            array[tam_array++] = currShow.clone();
        }
    }

    // Parte 2 - Pesquisa sequencial pelos títulos
    File registro = new File("./859238_sequencial.txt"); // troca para SEQUENCIAL
    long inicio = 0, fim = 0;
    int comparacoes = 0, movimentos = 0; // movimentos é zero para pesquisa

    try {
        FileWriter registros = new FileWriter(registro);

        inicio = new Date().getTime(); // pega o tempo inicial
        ...
        while (true) {
            ...
            for (int i = 0; i < tam_array; i++) {
                comparacoes++;
                if (array[i].title != null && array[i].title.equals(titulo)) {
                    found = true;
                    break;
                }
            }
        ...
        fim = new Date().getTime(); // pega o tempo final
        long duracao = fim - inicio; // calcula a duração
