...
void ordenaBolha(Show *shows, int count, int *mov, int *comp)
{
  for (int i = 0; i < count - 1; i++)
  {
    for (int j = 0; j < count - i - 1; j++)
    {
      (*comp)++;
      if (shows[j].date_added.year > shows[j + 1].date_added.year ||
          (shows[j].date_added.year == shows[j + 1].date_added.year && shows[j].date_added.month > shows[j + 1].date_added.month) ||
          (shows[j].date_added.year == shows[j + 1].date_added.year && shows[j].date_added.month == shows[j + 1].date_added.month && shows[j].date_added.day > shows[j + 1].date_added.day) ||
          (shows[j].date_added.year == shows[j + 1].date_added.year &&
           shows[j].date_added.month == shows[j + 1].date_added.month &&
           shows[j].date_added.day == shows[j + 1].date_added.day &&
           cmp(shows[j].title, shows[j + 1].title) > 0))
      {
        Show temp = shows[j];
        shows[j] = shows[j + 1];
        shows[j + 1] = temp;
...
{
  FILE *log_file = fopen("859238_bolha.txt", "a");
  if (log_file == NULL)
  {
...
ordenaBolha(selected, selected_count, &mov, &comp);
clock_t fim = clock();
double duration = (double)(fim - inicio) / CLOCKS_PER_SEC;

FILE *log = fopen("859238_bolha.txt", "w");
if (log) {
  fprintf(log, "859238\t%d\t%d\t%.6f\n", comp, mov, duration * 1000);
  fclose(log);
}
