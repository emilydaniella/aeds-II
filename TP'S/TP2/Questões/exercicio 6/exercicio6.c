...
Arquivo de Log: matricula_selecao.txt (contendo: matrícula, comparacoesarações, movimentosimentações, tempo de execução separados por \t)
*/
...
void selecaoRecursivo(Show *array, int inicio, int tamanho, int *comparacoes, int *movimentos) {
    ...
    for (int i = inicio + 1; i < tamanho; i++) {
        (*comparacoes)++;
        if (cmp(array[i].title, array[menor].title) < 0) {
            menor = i;
        }
    }
...
{
  FILE *log_file = fopen("859238_selecao.txt", "a");
  if (log_file == NULL)
  {
...
FILE *log = fopen("859238_selecao.txt", "w");
if (log) {
